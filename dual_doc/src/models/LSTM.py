...
class LSTM_CommonModule(nn.Module):
    ...
    def __init__(self, config, hidden_size, output_modules = 1):
        ...
        if output_modules == 1:
            ...
            bili_list = [PredictionBiLinear(hidden_size + dis_size, hidden_size + dis_size, config.relation_num) for _ in
                         range(output_modules)]
...
class LSTM(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.light:
            self.output_net = LSTM_CommonModule(config, hidden_size, output_modules= self.num_output_module)
        else:
            output_net_list = [LSTM_CommonModule(config, hidden_size) for _ in range(self.num_output_module)]
    ...
    def forward(self, context_idxs, pos, context_ner, context_char_idxs, context_lens, h_mapping, t_mapping,
        ...
        output = [self.output_net_list[i](context_output, h_mapping, t_mapping, dis_h_2_t, dis_t_2_h) for i in
