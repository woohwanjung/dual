...
from models.common_modules import PredictionLinear
from utils import from_cpu_to_cuda, from_cpu_to_cuda_list


class ContextAwareCommonModule(nn.Module):
    def __init__(self, config, hidden_size, output_modules = 1):
        super().__init__()
        self.config = config
        #self.num_output_modules = self.config.num_output_module
        ...
        if output_modules == 1:
            self.linear_output = PredictionLinear(hidden_size * 2, config.relation_num)
        else:
            li_list = [torch.nn.Linear(hidden_size * 2, config.relation_num) for _ in range(output_modules)]
            self.li_list = nn.ModuleList(li_list)

    def fix_prediction_bias(self, bias):
        assert self.num_output_modules == 1
        self.linear_output.fix_bias(bias)

    def add_prediction_bias(self, bias):
        assert self.num_output_modules == 1
        self.linear_output.add_bias(bias)

    def forward(self, sent, h_mapping, t_mapping, dis_h_2_t, dis_t_2_h, relation_mask):
        context_output = sent
        ...
        output = [self.li_list[i](re_rep) for i in range(self.num_output_modules)]
        return output
...
class ContextAware(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.num_output_module = config.num_output_module
        ...
        #self.output_net = ContextAwareCommonModule(config, hidden_size)

        '''
        self.output_net_list = [ContextAwareCommonModule(config, hidden_size) for _ in range(self.num_output_module)]
        ...
        #'''
        if self.light:
            self.output_net = ContextAwareCommonModule(config, hidden_size, output_modules= self.num_output_module)
        else:
            output_net_list = [ContextAwareCommonModule(config, hidden_size) for _ in range(self.num_output_module)]
    ...
    def forward(self, context_idxs, pos, context_ner, context_char_idxs, context_lens, h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h):
        ...
        if device != context_idxs.device:
            context_idxs, pos, context_ner, context_char_idxs, context_lens, h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h \
                = from_cpu_to_cuda_list([context_idxs, pos, context_ner, context_char_idxs, context_lens, h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h],device)
        ...
        if self.light:
            output = self.output_net(context_output, h_mapping, t_mapping, dis_h_2_t, dis_t_2_h, relation_mask)
            return output

        #'''
        output = [self.output_net_list[i](context_output, h_mapping, t_mapping, dis_h_2_t, dis_t_2_h, relation_mask) for i in
