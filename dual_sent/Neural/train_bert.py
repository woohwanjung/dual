...
from models.sbert import PRED_MODEL_BILINEAR
from utils import Dataset, DatasetHD, DatasetHDCrossIter, DatasetBERT

torch.backends.cudnn.deterministic = True


def train(args):
	model = Model(args, device, train_dset.rel2id)
	...
	for iter in range(niter):
		...
		for idx, batch in enumerate(tqdm(train_dset.batched_data,ascii=True)):
			loss_batch = model.update(batch, col_label=3, col_isha = 5)
			loss += loss_batch
		...
		valid_loss, (dev_prec, dev_recall, dev_f1) = model.eval(dev_dset, col_label=3, col_origidx=4,ds = True)
		logging.info(
			'Dev loss: {:.4f}, P: {:.4f}, R: {:.4f}, F1: {:.4f}'.format(valid_loss, dev_prec, dev_recall,
																				  dev_f1))

		valid_loss_c, (cdev_prec, cdev_recall, cdev_f1) = model.eval(cdev_dset, col_label=3, col_origidx=4)
		logging.info(
		...
		test_loss, (test_prec, test_recall, test_f1) = model.eval(test_dset, col_label=3, col_origidx=4, ds = True)
		logging.info(
...
if __name__ == '__main__':
	...
	parser.add_argument("--crossiter", type = bool, default = False)
	...
	parser.add_argument("--pred_model", type = int, default = PRED_MODEL_BILINEAR)
	...
	parser.add_argument('--lr', type=float, default=1.0, help='Applies to SGD, Adam and Adagrad.')
	...
	train_ds_dset = DatasetBERT(train_filename, args, word2id, device, rel2id=rel2id, shuffle=True)
