...
class PositionAwareRNN(nn.Module):
	...
	def __init__(self, opt, rel2id, emb_matrix=None):
		...
		opt['attn'] = True
		self.dual = opt['dual']
		self.multitask = opt['multitask']
		self.union = opt['union']
		...
		if (self.dual or self.multitask) and not self.union:
			if self.dual:
				self.num_output_modules = 4
			elif self.multitask:
				self.num_output_modules = 2
		
			flinear_list = [nn.Linear(opt['hidden'], len(rel2id)) for _ in range(self.num_output_modules)]
		...
		if (self.dual or self.multitask) and not self.union:
			for mi in range(self.num_output_modules):
				...
				if mi == 1:
					...
					init.xavier_uniform(flinear.weight, gain=1)
		else:
	...
	def forward(self, inputs):
		...
		seq_lens = masks.data.eq(utils.PAD_ID).long().sum(1)
