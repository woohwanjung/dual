...
class BGRU(nn.Module):
	def __init__(self, args, rel2id, word_emb=None):
		...
		# arguments
		hidden, vocab_size, emb_dim, pos_dim, ner_dim, position_dim, attn_dim, num_layers, dual = \
			args.hidden, args.vocab_size, args.emb_dim, args.pos_dim, args.ner_dim, \
			args.position_dim, args.attn_dim, args.num_layers, args.dual
		...
		if (self.dual or self.multitask) and not self.union:
			...
			flinear_list = [nn.Linear(hidden*2, len(rel2id)) for _ in range(self.num_output_modules)]
