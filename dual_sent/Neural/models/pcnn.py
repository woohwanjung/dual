...
class PCNN(nn.Module):
	def __init__(self, args, rel2id, word_emb=None):
		...
		if self.dual or self.multitask:
			...
			flinear_list = [nn.Linear(hidden*3, len(rel2id)) for _ in range(self.num_output_modules)]
			self.flinear_list = nn.ModuleList(flinear_list)
			for mi in range(self.num_output_modules):
				flinear = self.flinear_list[mi]
				if mi == 1:
					flinear.load_state_dict(self.flinear_list[0].state_dict())
				else:
					torch.nn.init.xavier_uniform_(flinear.weight.data)
					flinear.bias.data.zero_()


		else:
			self.flinear = nn.Linear(hidden*3, len(rel2id))
			# self.flinear.weight.data.normal_(std=0.001)
			torch.nn.init.xavier_uniform_(self.flinear.weight.data)
			self.flinear.bias.data.zero_()
	...
	def forward(self, inputs):
		...
		if self.dual or self.multitask:
			logits = [flinear(output) for flinear in self.flinear_list]
		else:
